...
class TrainSet(Dataset):
    def __init__(self, fname='./dataset/train.txt'):
        super(TrainSet, self).__init__()
        # self.raw_data, self.entity_dic, self.relation_dic = self.load_texd()
        self.raw_data, _, _ = self.load_text(fname)
        _, self.entity_to_index, self.relation_to_index = self.load_text()
    ...
    def load_text(self, fname='./dataset/train.txt'):
        raw_data = pd.read_csv(fname, sep='\t', header=None,
...
class TestSet(Dataset):
    ...
    def load_text(self):
        raw_data = pd.read_csv('./dataset/test.txt', sep='\t', header=None,
                               names=['head', 'relation', 'tail'],
                               keep_default_na=False, encoding='utf-8')
        raw_data = raw_data.applymap(lambda x: x.strip())
